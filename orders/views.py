...
def place_orders(request):
    ...
    for i in cart_items:
        ...
        total_data.update({fooditem.vendor.id: {subtotal: tax_dict}})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            order.total_data = dumps(total_data)
            order.save()
            order.order_number = generate_order_number(order.id)
            order.vendors.add(*vendors_ids)  # star because it will add the data to ManyToManyField ( to vendors field )
            order.save()
            context = {
                'order': order,
...
def payments(request):
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        for i in ordered_food:
            customer_subtotal += float(i.amount)
        ...
        context = {
            'user': request.user,
            'order': order,
            'to_email': order.email,  # this not need to be user's email address
